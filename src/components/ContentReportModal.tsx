...
/**
 * Modal de reporte de una pieza de CONTENIDO de creadora (PR-9). Espeja
 * `post/ReportModal` pero postea `{ category, description }` a
 * `/api/content/{id}/report`. El content_id + reporter (id/IP) los pone el server
 * — el cliente nunca los manda. localStorage es un soft-guard (el server tiene el
 * rate-limit real + dedup).
 */
const REPORT_CATEGORIES = [
  { value: 'illegal',        label: 'Contenido ilegal' },
  { value: 'dmca',           label: 'Infracción de derechos (DMCA)' },
  { value: 'nonconsensual',  label: 'Contenido no consentido' },
  { value: 'csam_suspected', label: 'Sospecha de menor (CSAM)' },
  { value: 'spam',           label: 'Spam' },
  { value: 'other',          label: 'Otro' },
]

export default function ContentReportModal({ contentId, onClose }: Props) {
  ...
  const submit = async () => {
    ...
    const LS_KEY = `report_content_${contentId}`
    ...
    setError(null)
    try {
      const res = await fetch(`/api/content/${encodeURIComponent(contentId)}/report`, {
        method: 'POST',
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
              {REPORT_CATEGORIES.map(opt => (
                <label key={opt.value} style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }}>
                  <input
                    type="radio" name="content-report-cat" value={opt.value}
